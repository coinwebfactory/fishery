                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 FISHERY ought to be enough for anybody." - Bill Gates, 2017
    ui->FISHERYpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelFISHERYSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintFISHERY->setEnabled(false);
        ui->pushButtonMintFISHERY->setToolTip(tr("zFISH is currently disabled due to maintenance."));

        ui->pushButtonSpendFISHERY->setEnabled(false);
        ui->pushButtonSpendFISHERY->setToolTip(tr("zFISH is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->FISHERYpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintFISHERY_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zFISH is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zFISH...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zFISH in ") +
...
void PrivacyDialog::on_pushButtonSpendFISHERY_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zFISH is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn FISHERY
        sendFISHERY();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send FISHERY
    sendFISHERY();
}

void PrivacyDialog::on_pushButtonZSubControl_clicked()
{
    ZSubControlDialog* zSubControl = new ZSubControlDialog(this);
    zSubControl->setModel(walletModel);
    zSubControl->exec();
}

void PrivacyDialog::setZSubControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzSubSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendFISHERY()
{
    QSettings settings;
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Fishery Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->FISHERYpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as FISHERY is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " FISH </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zFISH</b>";
    ...
    // use mints from zFish selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZSubControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZSubControlDialog::GetSelectedMints();
    }
    ...
    // Clear zfish selector in case it was used
    ZSubControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zFish Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Sub, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zFish Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zFISH </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zFISH "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zFISH "));
    ui->labelFISHERYAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelFISHERYSyncStatus->setVisible(fShow);
}
