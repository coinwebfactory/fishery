...
#include "ui_fisherycontroldialog.h"
...
std::list<std::string> ZSubControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZSubControlDialog::listMints;

ZSubControlDialog::ZSubControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZSubControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZSubControlDialog::~ZSubControlDialog()
{
    delete ui;
}

void ZSubControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZSubControlDialog::updateList()
{
...
void ZSubControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZSubControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZSub_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZSubControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZSubControlDialog::GetSelectedMints()
{
...
void ZSubControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
